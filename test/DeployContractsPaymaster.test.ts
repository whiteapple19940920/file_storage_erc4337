...
describe("Test Contract Deploy", function () {
    ...
    beforeEach(async function () {
        ...
        paymasterToken = await deployNew("PayMasterToken", [ entryPointAddress ]);
    ...
    it("WalletFactory Deploy Smart Wallet And Paymaster", async function () {
        ...
        const codeSize = await ethers.provider.getCode(expectedWalletAddress);

        expect(codeSize.length).to.be.gt(0);
        expect(await deployedWallet.owner()).to.be.eq(ownerAddress);
        expect(await deployedWallet.entryPoint()).to.be.eq(entryPoint.address);
    ...
    it("Test Paymaster", async function () {
        const initialWalletEthBalance = await ethers.provider.getBalance(sender);
        const initialPaymasterDeposit = await paymaster.getDeposit();
        expect(initialPaymasterDeposit).to.be.gt(ethers.BigNumber.from("0"));
        ...
        const payload = mockERC20.interface.encodeFunctionData(
            "transfer",
            [await owner.getAddress(), 10]
        );
